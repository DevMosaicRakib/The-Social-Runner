...
export default function EventsNearMeMap() {
  ...
  // Fetch nearby events using current location, radius, and time filter
  const { data: nearbyEvents, isLoading: isLoadingEvents } = useQuery<EventWithDistance[]>({
    queryKey: ["/api/events/search/location", userLocation.lat, userLocation.lng, radiusFilter, hoursFromFilter],
    queryFn: async () => {
      const url = `/api/events/search/location?lat=${userLocation.lat}&lng=${userLocation.lng}&radius=${radiusFilter}&hours=${hoursFromFilter}`;
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-600">
          Within {radiusFilter}km of {userLocation.label} • {

...
export function setupEmailAuth(app: Express) {
  ...
  app.post('/api/auth/register', async (req, res) => {
    try {
      ...
      // Email verification can be implemented when SENDGRID_API_KEY is configured
      ...
      const sessionUser = {
        claims: {
          ...
          profile_image_url: newUser.profileImageUrl || null,
        },
        expires_at: Math.floor(Date.now() / 1000) + (7 * 24 * 60 * 60), // 7 days
        access_token: null, // Not applicable for email auth
        refresh_token: null, // Not applicable for email auth
      };

      // Skip auto-login for now, just return success
      res.status(201).json({ 
  ...
  app.post('/api/auth/forgot-password', async (req, res) => {
    try {
      ...
      // Password reset email can be implemented when SENDGRID_API_KEY is configured
      if (process.env.SENDGRID_API_KEY) {
